...
import dev.woody.ext.Bus.mains.MainActivity;
import dev.woody.ext.ViewPagerFragment.ViewPager_Main;
...
public class Mains extends Activity{
    ...
    String createTable = "create table codehistory (code text);";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(info_flag == 0) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    //인포 인텐트
                    Intent intent = new Intent(getApplicationContext(), ViewPager_Main.class);
                    startActivity(intent);
                }
            }).start();
        }

        Log.d("pid_mains", "" + android.os.Process.myPid());
        Log.d("uid_m",""+ android.os.Process.myUid());
        ...
        if(isNetwork() == true) {
            ...
            Toast.makeText(this, "네트워크 연결 필요", Toast.LENGTH_SHORT).show();

                AlertDialog.Builder dialog = new AlertDialog.Builder(this);
                dialog.setMessage("어플리케이션이 종료됩니다").setCancelable(false).setPositiveButton("종료",
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if( pos == 0){
                ...
                if(isNetwork()) {
                    ...
                    Toast.makeText(getApplicationContext(), "네트워크 연결 필요", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    public void intent_web(String vcode) throws IOException {
        vcode_goto = vcode;

        if(vcode.equals("8282")){
            bus_flag = 1;
            insertData(vcode);
            Intent intent = new Intent(this,MainActivity.class);
            new gotoTask().execute();
            startActivity(intent);
        } else {
            bus_flag = 0;
            insertData(vcode);
    ...
    private void insertData(String title){
        ...
        try{
            ...
            Cursor cursor;
            String del = "delete from codehistory where code='"+ title +"'";
            String sel = "select code from codehistory where code='"+ title +"'";
            cursor = database.rawQuery(sel,null);
    ...
    private boolean isNetwork(){
        ...
        boolean isMobileActivity = connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).isAvailable();
        boolean isMobileConnect = connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).isConnected();
        boolean isWifiAvailable = connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_WIFI).isAvailable();
        boolean isWifiConnect = connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_WIFI).isConnected();

        if((isWifiAvailable && isWifiConnect) || (isMobileActivity && isMobileConnect)){
            return true;
        }else{
            return false;
        }
    ...
    class gotoTask extends AsyncTask<Void, Void, String> {
        String urlPath = "http://incoders.co.kr/incode/goto.php";
        @Override
        protected String doInBackground(Void... voids) {
            // TODO Auto-generated method stub
            try {
                ...
                Vector<NameValuePair> nameValue = new Vector<NameValuePair>();
                String code = vcode_goto;
                Log.d("codeA", code);
                String id = UserID;

                nameValue.add(new BasicNameValuePair("user_id", id));
                nameValue.add(new BasicNameValuePair("vcode", code));
        ...
        protected void onPostExecute(String value) {
            ...
            if(bus_flag == 0) {
                if(value.equals("not exist")) {
                    Toast.makeText(getApplicationContext(),"잘못된 코드입니다",Toast.LENGTH_LONG).show();
                }
                else{
